...
```
[毎日 JST 9:15]
run-daily.sh → claude -p (SKILL.md) → 記事収集・要約 → content/posts/YYYY-MM-DD.md → git push
...
run-improve.sh
    ...
    └── claude -p (skill-creator) → feedback.json + SKILL.md 分析 → SKILL.md 改善
    ↓
git commit & push → 次の記事収集に反映
```
