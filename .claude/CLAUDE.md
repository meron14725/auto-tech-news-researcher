...
Automated tech news aggregation site. Collects trending articles from multiple sources daily, summarizes them in Japanese using Claude Code (headless mode via `claude -p`), orchestrated by OpenClaw, and publishes to a Hugo static site on Cloudflare Pages.

## Architecture

> アーキテクチャ図: [docs/architecture.svg](../docs/architecture.svg)
> Claude Code 連携調査: [docs/claude-code-channels調査.md](../docs/claude-code-channels調査.md)

- **Orchestration**: OpenClaw Gateway on Hetzner Cloud CX33 (4 vCPU / 8 GB RAM, Ubuntu 24.04) — cron scheduling & task management only
- **AI Processing**: Claude Code headless (`claude -p`) for article collection, filtering, and Japanese summarization (Max 5x subscription)
...
- **Backup LLM**: MiniMax M2.5 via Token Plan Starter ($10/月) — available for other OpenClaw tasks

## Pipeline Flow

1. OpenClaw cron triggers at JST 9:15
2. OpenClaw executes research-trend-news skill
3. Skill calls `scripts/research-tech-news.sh` → `claude -p` (Claude Code headless)
4. Claude Code fetches from news sources (RSS/API via curl), filters by interest_score >= 7, generates Japanese summaries
5. Outputs `YYYY-MM-DD.md` (前日の日付、YAML front matter with articles array) and updates `processed_urls.json`
6. Skill calls `scripts/run-daily.sh` for git pull → add → commit → push
...
- Claude Code: Max 5x subscription covers headless usage (daily news collection = ~1 prompt/day)
- MiniMax Token Plan Starter: $10/月固定（1,500 req/5h、OpenClaw の他タスク用に維持）
- 月額合計: Hetzner $6.43 + MiniMax $10 = 約 $16.43/月（Claude Code は Max 5x に含む）
